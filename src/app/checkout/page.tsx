...
export default function CheckoutPage() {
  ...
  if (totalItems === 0 && !order) {
    return (
      <main className="max-w-2xl mx-auto px-6 py-24 text-center">
        ...
        <Link
          href="/produtos"
          className="inline-block bg-gold text-background-primary px-8 py-3 text-sm font-medium hover:opacity-90 transition-opacity"
        >
          Ver Produtos
        </Link>
      </main>
  ...
  // PIX QR code view after order placed
  if (order) {
    ...
    return (
      <main className="max-w-lg mx-auto px-6 py-16 text-center">
        ...
        <div className="border border-border p-6 mb-6 bg-background-secondary">
          ...
          {pixCode ? (
            <>
              <div className="flex justify-center mb-4 bg-white p-4 inline-block mx-auto">
            ...
            <p className="text-text-secondary text-sm">
              Entre em contato via WhatsApp para receber a chave PIX.
            </p>
          )}
        </div>
  ...
  return (
    <main className="max-w-7xl mx-auto px-6 py-16">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
        {/* Cart summary */}
        <section>
          <h2 className="font-heading text-text-primary text-xl mb-6 pb-3 border-b border-border">
            Resumo do Carrinho
          </h2>
              ...
              <li key={item.productId} className="flex items-center gap-4">
                ...
                <div className="flex items-center border border-border shrink-0">
                  <button
                    ...
                    className="px-2 py-1 text-text-primary hover:text-gold transition-colors text-sm"
                  >
                    −
                  </button>
                  <span className="px-3 py-1 text-text-primary text-sm min-w-[2rem] text-center">
                    {item.quantity}
                  </span>
                  <button
                    onClick={() => updateQuantity(item.productId, item.quantity + 1)}
                ...
                <p className="text-gold font-semibold text-sm shrink-0 w-20 text-right">
